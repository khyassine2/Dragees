...
export const Cursor = () => {
  ...
  return (
    <div aria-hidden className="pointer-events-none fixed inset-0 z-200">
      <div
        ref={ringRef}
        className="absolute -top-5 -left-5 flex size-10 items-center justify-center rounded-full border border-plum/45 opacity-0 will-change-transform"
      >
        <span
          ref={labelRef}
          className="label-micro scale-[0.55] text-ivory opacity-0"
        />
      </div>
      <div
        ref={dotRef}
        className="absolute -top-[3px] -left-[3px] size-1.5 rounded-full bg-plum opacity-0 will-change-transform"
      />
    </div>
  );
};
